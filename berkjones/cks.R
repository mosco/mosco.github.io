# source("C:/Users/amitmo/Dropbox/git_repos/cks-paper/ckslib/R/CKS.R")
...
cks <- function(uniform01_samples)
    # cks(uniform01_samples) -> CKS statistic (two sided)
    # 
    # Compute the two sided CKS test statistic for a list of samples assumed
    ...
    # 
    # Returns:
    #     The two-sided CKS statistic defined in Eq. (3.2) of the paper
    #         http://arxiv.org/pdf/1311.3190v1.pdf
{
    p_values <- compute_p_values(uniform01_samples) 
    return(min(min(p_values), 1.0-max(p_values)))
}

cks_plus <- function(uniform01_samples)
    # cks_plus(uniform01_samples) -> CKS+ score (one-sided sided)
    # 
    # Like the cks() function, but for testing deviations towards lower values only.
{
    p_values <- compute_p_values(uniform01_samples) 
    return(min(p_values))
}

cks_minus <- function(uniform01_samples)
    # cks_minus(uniform01_samples) -> CKS- score (one-sided sided)
    # 
    # Like the cks() function, but for testing deviations towards higher values only.
{
    p_values <- compute_p_values(uniform01_samples) 
